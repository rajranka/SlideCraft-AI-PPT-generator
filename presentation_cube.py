...
def create_slides_cube(gpt_response, slide_title, slide_number, Topic, choice, pic):
    prs = Presentation()
    #Topic Slide
    topic = prs.slides.add_slide(prs.slide_layouts[5])
    background = topic.shapes.add_picture("Templates\Cube\Cube_Title_Slide.png", 0, 0, prs.slide_width, prs.slide_height)
    #Topic Title
    ...
    ctf1.paragraphs[0].font.color.rgb = RGBColor(0, 0, 0)
    ...
    for i in range(slide_number):
        ...
        if count<no_of_pic:
            if i%2==0:
                background = slide.shapes.add_picture("Templates\Cube\Cube_Slide_"+str(i+1)+".png", 0, 0, prs.slide_width, prs.slide_height)
                ...
                count = count + 1
                background = slide.shapes.add_picture("Templates\Cube\Cube_Picture_Slide_"+str(count)+".png", 0, 0, prs.slide_width, prs.slide_height)
                content = slide.shapes.add_textbox(Pt(60), Pt(160), Pt(370), Pt(300))
                ctf2 = content.text_frame
                if choice == 1:
                    content.text = slide_content
                elif choice == 2:
